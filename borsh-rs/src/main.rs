...
mod test {
    ...
    fn test_no_schema() {
        ...
        let my_example_struct = MyExampleStruct {
            name: "Albert".to_string(),
            age: 9,
        };

        fn serialize_my_example_struct(my_example_struct: MyExampleStruct) -> Vec<u8> {
            let ser_my_example_struct: Vec<u8> = my_example_struct.try_to_vec().unwrap();
    ...
    fn test_with_schema() {
        ...
        println!("{:?}", ser_my_example_struct);
        // Deserialize this instance from a slice of bytes, but assume that at the beginning we have bytes describing the schema of the type
        let deser_ser_my_example_struct = try_from_slice_with_schema::<MyExampleStruct>(&ser_my_example_struct).unwrap();
        println!("{:?}", deser_ser_my_example_struct);
        // assert
        assert_eq!(deser_ser_my_example_struct, my_example_struct);
    }
